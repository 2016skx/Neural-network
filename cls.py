...
tot_num = 5000        #total number of training samples
learn_rate = 0.01     #learning rate

###
#@statement  read files from txt files, save to training set and test set respectively
#
...
###
#@statement   training set and test set batched by training batch size
...
saver = tf.train.Saver(write_version=tf.train.SaverDef.V2) # statement tf.train.Saver to save model
...
with tf.Session() as sess:
    ...
    for iter in range(iteration):
        ...
        train = sess.run([precision,recall,loss], feed_dict={x: x_train_batch, y_:y_train_batch, keep_prob:0.5})
        test = sess.run([precision,recall,loss], feed_dict={x: x_test_batch, y_:y_test_batch, keep_prob:1.0})
        saver_path = saver.save(sess, "save/tensor_net.ckpt")  # save the model as save/model.ckpt
